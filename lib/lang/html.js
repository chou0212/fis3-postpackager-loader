...
function process(file, resource, opts) {
    ...
    if (resource.css.length) {
      ...
      resource.css.forEach(function(item) {
        if (item.embed) {
          ...
          preload.push('<link rel="preload" href="' + msg.ret + '" as="style" />');
    ...
    if (resource.js.length) {
      ...
      resource.js.forEach(function(item) {
        if (item.embed) {
          ...
          preload.push('<link rel="preload" href="' + msg.ret + '" as="script" />');
  ...
  resource.asyncs.forEach(function(item) {
    preload.push('<link rel="preload" href="' + item.uri + '" as="script" />');
  });
  
  // 插入preload资源
  if (opts.preload) {
    content = content.replace(new RegExp(opts.preloadPlaceHolder, 'gm'), '    ' + preload.join('\n    '));
  }
